...
namespace xcore
{
	s32	xnetip::to_string(xuchars& str, bool omit_port) const
	{
		...
		else if (m_type == NETIP_IPV6)
		{
			...
			if (omit_port)
			{
				ascii::sprintf(str, xcuchars("%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x"), x_va(m_ip.ip16[0]), x_va(m_ip.ip16[1]), x_va(m_ip.ip16[2]), x_va(m_ip.ip16[3]), x_va(m_ip.ip16[4]), x_va(m_ip.ip16[5]), x_va(m_ip.ip16[6]), x_va(m_ip.ip16[7]));
				l = 1;
			}
			else
			{
				ascii::sprintf(str, xcuchars("[%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x]:%u"), x_va(m_ip.ip16[0]), x_va(m_ip.ip16[1]), x_va(m_ip.ip16[2]), x_va(m_ip.ip16[3]), x_va(m_ip.ip16[4]), x_va(m_ip.ip16[5]), x_va(m_ip.ip16[6]), x_va(m_ip.ip16[7]), x_va(m_port));
				l = 1;
			}
		}
		return l;
